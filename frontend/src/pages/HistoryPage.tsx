...
import {
  ...
  ChevronRight, FileText, AlertTriangle, BarChart3, Lightbulb, Download,
  Cpu, Unplug,
} from 'lucide-react';
...
const provenanceChip = (s: HistorySession) => {
  const src = s.feedbackSource;
  if (src === 'ai') {
    return {
      icon: Cpu,
      label: s.provider ? `${s.provider}${s.model ? ` · ${s.model}` : ''}` : 'AI feedback',
      color: 'hsl(174 85% 60%)',
      title: 'Generated by a live AI provider',
    };
  }
  if (src === 'fallback') {
    return {
      icon: AlertTriangle,
      label: 'Derived feedback',
      color: 'hsl(35 90% 55%)',
      title: s.fallbackReason || 'Generated locally (AI output invalid)',
    };
  }
  if (src === 'mock') {
    return {
      ...
      label: 'Offline mode',
      color: 'hsl(210 10% 45%)',
      title: 'AI provider unavailable — deterministic feedback',
    };
  }
  return null;
};
...
export default function HistoryPage() {
    ...
    return records
      ...
      .map((s) => {
        ...
        return {
          ...
          fallbackReason: feedback?.fallbackReason || null,
          resumeFileUrl: s.resumeFileUrl || null,
  ...
  const downloadResume = async (s: HistorySession) => {
    if (!s.resumeFileUrl) return;
    setDownloadingResume(s.id);
    try {
      const res = await apiFetch(s.resumeFileUrl);
      if (!res.ok) throw new Error(`Download failed (${res.status})`);
      const blob = await res.blob();
      const ext = /\.pdf$/i.test(s.resumeFileUrl) ? 'pdf' : 'txt';
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = s.resumeFileName || `resume.${ext}`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      setTimeout(() => URL.revokeObjectURL(a.href), 1000);
    } catch (err) {
      console.error('[History] resume download failed:', err);
    } finally {
                ...
                >
                  ...
                  <div style={{ flex: 1, overflow: 'hidden' }}>
                    ...
                    <p style={{ fontSize: 12, color: 'hsl(210 10% 48%)', display: 'flex', alignItems: 'center', gap: 6, marginTop: 2, flexWrap: 'wrap' }}>
                      ...
                      {(() => {
                        const pc = provenanceChip(s);
                        if (!pc) return null;
                        const Icon = pc.icon;
                        return (
                          <span title={pc.title} style={{
                            display: 'inline-flex', alignItems: 'center', gap: 4,
                            fontSize: 10, fontWeight: 600, letterSpacing: '0.03em',
                            padding: '2px 8px', borderRadius: 999,
                            color: pc.color,
                            background: `${pc.color}1a`,
                            border: `1px solid ${pc.color}35`,
                            textTransform: 'uppercase',
                          }}>
                            <Icon size={10} /> {pc.label}
                          </span>
                        );
                      })()}
                    </p>
                      ...
                      <div style={{
                        ...
                        <div style={{
                          ...
                          {s.resumeFileUrl && (
                            ...
                            >
                              <Download size={14} />
                              {downloadingResume === s.id ? 'Downloading…' : 'Download Resume'}
                            </button>
                          )}
                          <button
